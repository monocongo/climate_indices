...
grid (40,260 cells, 528 months, SPI-6 gamma), `main` runs the eager in-memory
...
`main` the pool's fixed cost still dominates at this size: the best Dask
configuration (4 workers) is 1.4x slower than the same call in memory. The
numbers, method, and retained raw output are in
