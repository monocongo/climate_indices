...
creates one from the host's physical memory, passes it to the calculation and the
Zarr write, and closes it on both the success and failure paths. The measured
fresh-pool versus warm-pool cost is in
[Operational Guidance](xarray_compatibility.md#operational-guidance).
