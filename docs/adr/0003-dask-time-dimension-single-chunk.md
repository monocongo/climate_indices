...
`notebooks/zarr_dask_spi_spei.ipynb` is the worked example: it keeps one time chunk from the prepared store through the calculation. `docs/troubleshooting.rst` lists the failure modes, including the `Prepared store must keep time as a single chunk.` error.
