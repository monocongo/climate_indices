...
`fad5cb8e` Hargreaves), because their kernels are per-day arithmetic rather than
...
Callers building Dask arrays for use with this library must chunk only along spatial dimensions (e.g. `{"lat": 50, "lon": 50, "time": -1}`), not time. The PET entry points are the exception: they accept chunked time and rechunk it internally.
