...
- **List issues**: `gh issue list --limit 200 --json number,title,body,labels,comments --jq '[.[] | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'`, narrowing with `--label`/`--state`/`--search`. `--limit` defaults to 30, which silently truncates a backlog this repo's size; 200 covers today's open-issue count but isn't unbounded — check first with `gh issue list --state open --limit 201 --json number --jq length`; a result of 201 means at least 201 open issues, so select a larger listing limit.
