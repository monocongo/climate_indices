...
| `edi(pe, data_start_year, calibration_year_initial, calibration_year_final, *, duration=365, spatial_time_major=False)` | effective precipitation in mm from the row above, and the calendar years bounding the Calibration Period | dimensionless EDI; `duration` is accepted for symmetry with PE but has no effect on the fixed-window EDI: the harmonic factor cancels in standardization |
