...
**Scope:** Whether the Western Regional Climate Center's (WRCC, at the Desert Research Institute) self-calibrated PDSI product can serve as independent external-product validation for `climate_indices.palmer.scpdsi()`. Written for [Assess DRI/WRCC scPDSI as an authoritative external validation dataset](https://github.com/monocongo/climate_indices/issues/780) under the map [Wayfinder: External authoritative-dataset validation for SPI, SPEI, PET, EDDI, and Palmer/scPDSI](https://github.com/monocongo/climate_indices/issues/769). External claims below rest on primary sources — the product's own pages and file metadata on `wrcc-archive.dri.edu`, the PRISM terms of use, the Crossref record for the product's citation (DOI 10.1175/BAMS-D-16-0193.1), and direct HTTP probes of the product's endpoints. Repository claims cite repository artifacts: ADR-0011, `src/climate_indices/palmer.py`, `VALIDATION.md`, `tests/`, and `tests/fixture/provenance_schema.json`. The BAMS article itself returned HTTP 403 from this environment, so no claim rests on its contents.
...
- The calibration window is stated in the NetCDF metadata: the file title says "calibrated to 1895-2010 for the continental United States", and the `data` variable description is "Self Calibrated Palmer Drought Severity Index (scPDSI), calibrated to 1895-2010" [netcdf]. The window is declared at the file level in the files inspected here, so in those files post-2010 values are calibrated against 1895-2010 rather than a rolling period; Unknown 5 records that persistence across all 3,155 artifacts is unverified.
...
- `scpdsi_current_PRISM.nc` (timestep 2025-10-15) has zero valid cells: all 872,505 cells hold the declared `_FillValue` (`-9999.0`), which standard readers decode to NaN. `scpdsi_2025_10_PRISM.nc` matches it in size, `Last-Modified`, and SHA-256 (`b21309d2e340fc22beeb74b4dd89e7e6b083c0fbd1906b03b82d40fcd9b9358b` for both files, downloaded 2026-09-18), so "current" is byte-identical to the last month's file, with the same all-missing mask. `scpdsi_1895_12_PRISM.nc`, by contrast, holds 476,109 valid values in the range -5.99 to 4.74 [netcdf].
- The time-series application was unavailable when probed on 2026-09-18: `https://wrcc-archive.dri.edu/wwdt/time/text/?lat=39.5&lon=-119.8&variable=SCPDSI&start_year=1895&end_year=2025` and its `regionText`/`PDSI` variants all returned HTTP 500 ("Page unavailable ... or the requested data may not exist"). The page's own backend contract is `.../time/text/?lat&lon&variable&start_year&end_year`, discovered in `media/js/interface.js` [time, probe].
...
- There is no versioned release, DOI, checksum manifest, or immutable object naming. Files are overwritten in place under fixed names, and the server exposes only Apache `Last-Modified` and `ETag` headers (for example `scpdsi_current_PRISM.nc`: `ETag "35666c-64145dfef6a83"`, `Last-Modified: Thu, 16 Oct 2025 12:37:05 GMT`, `Accept-Ranges: bytes`); neither header is an immutable release identifier [headers].
