...
1. **The product appears abandoned and the newest file is empty.** The newest file with valid data is September 2025; every file was last written 2025-10-16, eleven months before this research date. `scpdsi_2025_10_PRISM.nc` and `scpdsi_current_PRISM.nc` (the file the download page offers for current conditions) each contain nothing but the declared `_FillValue`, with zero valid cells, and neither has changed since. Every time-series endpoint probed (`/wwdt/time/text/`, `/wwdt/time/regionText/`) returned HTTP 500, and the data directory still exposes a legacy `phpinfo()` deployment. These observations are consistent with WWDT no longer being produced, but this note does not establish why it stopped; Unknown 1 records the question.
...
- The downloaded NetCDF files carry `author: John Abatzoglou - University of Idaho`. The 2025-vintage files carry `date: 16 October 2025`; the 1895-12 file carries `date: 10 December 2014`. WRCC/DRI is the host and legacy producer; the file-level author is now affiliated with the University of Idaho. [netcdf]
...
- The newest valid data is September 2025. The September series files and the current file carry `Last-Modified` values on 2025-10-16 between 12:36:00 and 12:37:05 GMT, and the 2025 per-year files were written monthly from 2025-07-05 through 2025-10-16. Nothing in the tree has changed since. Directory-listing timestamps are server-local (UTC-7), one hour behind the GMT dates shown here for the same files [listing, headers].
...
- The About page states data are "updated with new values at the beginning of each month" [about]. Observed behavior contradicts this by eleven months: all artifact timestamps stop at 2025-10-16, and the last two months' files are empty [listing, netcdf].
...
- Because files are mutable and unversioned, any acquisition must record the per-file SHA-256, `Last-Modified`, and ETag at download time, as `tests/fixture/provenance_schema.json` already requires for committed fixtures.
...
1. **Pilot.** Pick a small set of grid cells and a common period, download the relevant `scpdsi_{year}_{month}_PRISM.nc` files (3.3 MB each), and measure divergence before committing to a multi-gigabyte acquisition.
...
4. **Per-location execution.** ADR-0011 keeps `scpdsi()` on the per-location path and it rejects 3+-D inputs, so the grid must be driven one cell at a time with `ConvergenceError` handling per cell and the multi-cell route (ADR-0002 multiprocessing) for throughput. Budget for that cost in the pilot.
...
1. **Operational status.** Is WWDT still produced? The last valid data are September 2025, the following two months' files are fill-only, and the extractor service is down. Contact WRCC/DRI (or the file author, University of Idaho) before any acquisition work.
