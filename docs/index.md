...
In order to build and install the package from source we need to first install
...
* - Option
  - Description
* - index
  - Which of the climate indices to compute. Valid values are 'spi', 'spei', 'pnp', 'scaled', 'pet', 'palmers', 'kbdi', and 'all'. 'scaled' indicates all three scaled indices (SPI, SPEI, and PNP) and 'palmers' indicates all Palmer indices (PDSI, PHDI, PMDI, and Z-Index). KBDI is only available via 'kbdi', not 'all'.
* - periodicity
...
* - chunksizes
  - Chunking of the written output file, not of the computation: 'none' (default) lets the writer choose the output layout, and 'input' copies the precipitation variable's on-disk chunks to the output.
* - output_file_base
...
`process_climate_indices --index palmers` command; see
[issue #721](https://github.com/monocongo/climate_indices/issues/721).
:::
