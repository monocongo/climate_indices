...
| Spatial (gridded) kernels | Yes, for SPI and SPEI | `spi`/`spei` receive a time-major `(time, *cells)` block, declared by the adapter, and fit every cell in one pass, so a gridded gamma run costs one kernel call per Dask block instead of one call per cell ([ADR-0008](adr/0008-spatial-block-declaration.md)). The NumPy API still accepts 1-D and 2-D arrays only. Inputs with a single non-core dimension keep the per-cell path, as do EDDI and percentage-of-normal ([#942](https://github.com/monocongo/climate_indices/issues/942)), PET ([#941](https://github.com/monocongo/climate_indices/issues/941)), and the Pearson Type III fit ([#940](https://github.com/monocongo/climate_indices/issues/940)). See `tests/test_spatial_kernel.py`. |
