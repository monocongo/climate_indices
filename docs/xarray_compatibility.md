...
| Dask-backed arrays | Yes, constrained | The time dimension must be a single chunk. Adapter tests verify detection and validation. Chunk shapes the CLI leaves to Dask (`"auto"`) resolve against a 100 MB array chunk budget rather than Dask's own default. |
...
- Chunk shapes the CLI entry points leave to Dask (`"auto"`) resolve against a 100 MB
  array chunk budget (`climate_indices._cli.DEFAULT_ARRAY_CHUNK_SIZE`), not Dask's own
  128 MiB default. Override it with explicit `chunks` when opening the input, with the
  `DASK_ARRAY__CHUNK_SIZE` environment variable, or with
  `dask.config.set({"array.chunk-size": ...})` before the CLI runs.
