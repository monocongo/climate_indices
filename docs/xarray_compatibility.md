...
| `DataArray` inputs for HDW | Yes | `fire.hot_dry_windy()` reduces the caller-named `level_dim` (default `"level"`) to the layer maximum via `xr.apply_ufunc`, calling its own NumPy path as the kernel; CF metadata comes from the `hdw` registry entry, and a CF `units` attribute on temperature is converted to Celsius. Not recursive: every other dimension, including `time` if present, is a plain passthrough with no cadence or alignment requirement. `level_dim` must be a single Dask chunk; other dimensions chunk freely. See `tests/test_fire.py`. |
