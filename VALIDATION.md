...
- **Independent implementation**: `tidyindex` (a harmonic-weight rolling
  z-score) and `rrk4910/EDI` (a weighted double-sum variant that omits the
  variable-duration standardization), plus the MIT `ahrapi` package (fixed-`k`
  API), implement related but non-identical algorithms; none reproduces this
...
- **Regression only**: what is committed. `tests/test_flood_reference.py`
  ...
  identity, the harmonic endpoint weights `w₁ = H_D` and `w_D = 1 / D`, and the
  `P / (1 − k)` constant-input limit, and freezes the public signatures. The
  per-index contract suites, run outside the `validation` marker, cover the
  Eq. (3) API recursion, the fixed-window EDI algebra, annual-maximum
