...
def _parse_soil_constants(soil_file):
    ...
    divs_to_awc = results_df[_AWC_VAR_NAME].to_dict()
    divs_to_lats = results_df['lat'].to_dict()
    
    return divs_to_awc, divs_to_lats

#-----------------------------------------------------------------------------------------------------------------------
def create_netcdf(nclimdiv_netcdf,
                  division_ids,
                  divisional_arrays,
    ...
    with netCDF4.Dataset(nclimdiv_netcdf, 'w') as dataset:
        ...
        time_variable.units = 'days since ' + str(p_min_year) + '-01-01 00:00:00'
        time_variable[:] = compute_days(p_min_year, time_steps)
        ...
        for var_name in variable_names:
            ...
            elif var_name == _PRECIP_VAR_NAME:
        ...
        awc_variable = dataset.createVariable(_AWC_VAR_NAME, float_type, (_DIVISION_VAR_NAME,), fill_value=filler)
...
#         nclimdiv_netcdf = '{0}_{1}.nc'.format(args.base_file_path, processing_date)
        ...
        divisional_arrays = {_TEMP_VAR_NAME: t_divs_to_arrays,
                             _PRECIP_VAR_NAME: p_divs_to_arrays}
        divisional_minmax_years = {_TEMP_VAR_NAME: t_divs_to_minmax_years,
                                   _PRECIP_VAR_NAME: p_divs_to_minmax_years}
    ...
    #     div_file = 'C:/home/climdivs/soil_constant_divisions.txt'
        ...
        print('\nResult file: {0}'.format(nclimdiv_netcdf))
