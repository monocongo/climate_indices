...
class EtoTestCase(fixtures.FixturesTestCase):
    ...
    def test_sunset_hour_angle(self):
        ...
        self.assertRaises(
            ValueError, eto._sunset_hour_angle, np.deg2rad(-100.0), np.deg2rad(0.0)
        )
        self.assertRaises(ValueError, eto._sunset_hour_angle, np.NaN, np.deg2rad(0.0))

        # make sure that an invalid solar declination angle raises an error
        self.assertRaises(
            ValueError, eto._sunset_hour_angle, np.deg2rad(0.0), np.deg2rad(-75.0)
        )
        self.assertRaises(
            ValueError, eto._sunset_hour_angle, np.deg2rad(0.0), np.deg2rad(85.0)
        )
        self.assertRaises(ValueError, eto._sunset_hour_angle, np.deg2rad(0.0), np.NaN)
    ...
    def test_solar_declination(self):
        ...
        self.assertRaises(ValueError, eto._solar_declination, 0)
        self.assertRaises(ValueError, eto._solar_declination, -1)
        self.assertRaises(ValueError, eto._solar_declination, 367)
    ...
    def test_daylight_hours(self):
        ...
        # make sure invalid arguments raise an error
        self.assertRaises(ValueError, eto._daylight_hours, math.pi + 1)
