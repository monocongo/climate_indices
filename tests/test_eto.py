...
def test_eto_thornthwaite(
    temps_celsius, latitude_degrees, data_year_start_monthly, pet_thornthwaite_mm
):

    # compute PET from the monthly temperatures, latitude, and initial years
    computed_pet = eto.eto_thornthwaite(
        temps_celsius, latitude_degrees, data_year_start_monthly
    )
    ...
    pytest.raises(
        TypeError, eto.eto_thornthwaite, temps_celsius, None, data_year_start_monthly
    )

    # make sure that an invalid latitude value (NaN) raises an error
    pytest.raises(
        ValueError, eto.eto_thornthwaite, temps_celsius, np.NaN, data_year_start_monthly
    )
