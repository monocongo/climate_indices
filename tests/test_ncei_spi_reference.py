...
# calibration (1895-2022). Loaded from provenance.json -- the single source of
# truth, regenerated by scripts/prepare_ncei_spi_fixtures.py.
_PROVENANCE = json.loads((_NCEI_SPI_ROOT / "provenance.json").read_text(encoding="utf-8"))
_MEASURED = {int(key[2:]): stats for key, stats in _PROVENANCE["measured_stats"].items()}
_CEILINGS: dict[int, dict[str, float]] = {}
for _key, _value in _PROVENANCE["validation_tolerance"].items():
    _scale, _stat = _key.split("_")
    _CEILINGS.setdefault(int(_scale[2:]), {})[_stat] = _value
...
def test_provenance_declares_all_scales():
    """provenance.json (source of the asserted constants) covers every tested scale."""
    assert set(_MEASURED) == set(_SCALES)
    assert set(_CEILINGS) == set(_SCALES)
