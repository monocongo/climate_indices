...
class UtilsTestCase(unittest.TestCase):
    ...
    def test_count_zeros_and_non_missings(self):
        ...
        self.assertEqual(
            non_missings, 12, "Failed to correctly count non-missing values"
        )
    ...
    def test_reshape_to_2d():
        ...
        np.testing.assert_raises(
            ValueError, utils.reshape_to_2d, values_2d.reshape((3, 3, 3)), 6
        )
    ...
    def test_reshape_to_divs_years_months():
        ...
        # verify that the function performed as expected
        np.testing.assert_raises(
            ValueError, utils.reshape_to_divs_years_months, values_1d
        )
        ...
        # verify that the function performed as expected
        np.testing.assert_equal(
            values_3d, values_3d_reshaped, "Not returning a valid 2-D array as expected"
        )
        ...
        np.testing.assert_raises(
            ValueError, utils.reshape_to_divs_years_months, values_2d
        )
        np.testing.assert_raises(
            ValueError,
    ...
    def test_transform_to_gregorian():
        ...
        np.testing.assert_raises(
            ValueError, utils.transform_to_gregorian, values_365, 1972
        )
        np.testing.assert_raises(
            ValueError, utils.transform_to_gregorian, np.ones((2, 10)), 1972
        )

        # make sure that the function croaks with a ValueError whenever it gets invalid year arguments
        np.testing.assert_raises(
            ValueError, utils.transform_to_gregorian, values_366, -1972
        )
        np.testing.assert_raises(
            TypeError, utils.transform_to_gregorian, values_366, 45.7
        )
        np.testing.assert_raises(
            TypeError, utils.transform_to_gregorian, values_366, "obviously wrong"
        )
    ...
    def test_transform_to_366day():
        ...
        # make sure that the function croaks with a ValueError whenever it gets invalid array arguments
        np.testing.assert_raises(
            ValueError, utils.transform_to_366day, values_365[:50], 1972, 1
        )
        np.testing.assert_raises(
            ValueError, utils.transform_to_366day, np.ones((2, 10)), 1972, 1
        )

        # make sure that the function croaks with a ValueError whenever it gets invalid year arguments
        np.testing.assert_raises(
            ValueError, utils.transform_to_366day, values_365, -1972, 1
        )
        np.testing.assert_raises(
            TypeError, utils.transform_to_366day, values_365, 45.7, 1
        )
        np.testing.assert_raises(
            TypeError, utils.transform_to_366day, values_365, "obviously wrong", 1
        )

        # make sure that the function croaks with a ValueError whenever it gets invalid total years arguments
        np.testing.assert_raises(
            ValueError, utils.transform_to_366day, values_365, 1972, -5
        )
        np.testing.assert_raises(
            TypeError, utils.transform_to_366day, values_365, 1972, 4.9
        )
        np.testing.assert_raises(
            ValueError, utils.transform_to_366day, values_365, 1972, 24
        )
