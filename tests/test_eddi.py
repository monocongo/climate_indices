"""Tests for EDDI (Evaporative Demand Drought Index) computation."""
...
def test_eddi_invalid_array_shape(
    ...
    """Test that 3-D+ arrays are rejected with DataShapeError."""
...
def test_eddi_invalid_periodicity(
    ...
    """Test that invalid periodicity raises InvalidArgumentError."""
    with pytest.raises(InvalidArgumentError, match="Invalid periodicity argument"):
        indices.eddi(
            pet_values=pet_thornthwaite_mm.flatten(),
            scale=6,
            data_start_year=data_year_start_monthly,
...
def test_eddi_empirical_ranking(
    ...
    computed_eddi = indices.eddi(
        ...
        calibration_year_initial=1902,
        calibration_year_final=1907,
        periodicity=compute.Periodicity.monthly,
    )
    ...
    # verify monotonic relationship (higher PET -> higher EDDI for January)
...
def test_eddi_calibration_end_after_data():
    """Test that calibration end year after data end year raises InvalidArgumentError."""
    num_years = 10
    pet_simple = np.ones(num_years * 12) * 100.0
    data_start_year = 2000
...
def test_eddi_calibration_start_after_end():
    """Test that calibration start year after end year raises InvalidArgumentError."""
    num_years = 10
    pet_simple = np.ones(num_years * 12) * 100.0
    data_start_year = 2000
...
def test_eddi_insufficient_climatology():
    ...
    # set ALL February values to NaN to force len(climatology_valid) < 2
    for year in range(num_years):
...
def test_eddi_mixed_nan_positions():
    ...
    # verify NaN positions in input produce NaN in output
    for pos in nan_positions:
