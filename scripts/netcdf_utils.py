...
# ----------------------------------------------------------------------------------------------------------------------
def variable_units(netcdf_file, var_name):
    """
...
# ----------------------------------------------------------------------------------------------------------------------
def variable_fillvalue(netcdf_file, var_name):
    """
...
def convert_and_move_netcdf(input_and_output_netcdfs):  # pragma: no cover
    ...
    try:
        ...
        nco = nco.Nco()
        nco.ncks(
            input=[input_netcdf, output_netcdf],
...
# ----------------------------------------------------------------------------------------------------------------------
def create_dataset_climdivs(
    file_path, division_ids, initial_year, total_months  # pragma: no cover
):
    """
    ...
    with netCDF4.Dataset(file_path, "w") as dataset:
        ...
        dataset.standard_name_vocabulary = (
        ...
        dataset.Conventions = (
            "ClimDiv-1.0"
        )  # suggested by Steve Ansari for support within the NOAA WCT
        ...
        divisions_variable = dataset.createVariable(
            "division", int_dtype, ("division",)
        )
        ...
        divisions_variable.setncatts(
            {"long_name": "US climate division ID", "standard_name": "division ID"}
        )

        # set the coordinate variables' values
        time_variable[:] = utils.compute_days(
            initial_year, total_months, 1, units_start_year
        )
        divisions_variable[:] = np.array(sorted(division_ids), dtype=np.dtype(int))


# ----------------------------------------------------------------------------------------------------------------------
def initialize_netcdf_divisions(
    file_path, template_netcdf, precip_var_name, variables, overwrite=False
):
    """
    ...
    for var_name, attributes in variables.items():
        variable = xr.Variable(
            dims=var_dims, attrs=attributes, data=np.empty(shape=dims_shape)
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def initialize_netcdf_single_variable_grid(
    ...
    with netCDF4.Dataset(template_netcdf, "r") as template_dataset:
        ...
        data_variable = dataset.createVariable(
            variable_name,
            data_dtype,
            ("lat", "lon", "time"),
...
# ----------------------------------------------------------------------------------------------------------------------
def initialize_dataset_climdivs(
    file_path,  # pragma: no cover
    template_dataset,
    divisions_dim_name,
    ...
    time_dtype = find_netcdf_datatype(template_dataset.variables["time"])
    divisions_dtype = find_netcdf_datatype(
        template_dataset.variables[divisions_dim_name]
    )

    # create the variables
    time_variable = netcdf.createVariable("time", time_dtype, ("time",))
    divisions_variable = netcdf.createVariable(
        divisions_dim_name, divisions_dtype, (divisions_dim_name,)
    )

    # set the variables' attributes
    time_variable.setncatts(template_dataset.variables["time"].__dict__)
    divisions_variable.setncatts(
        template_dataset.variables[divisions_dim_name].__dict__
    )
    ...
    if data_variable_name is not None:
        ...
        data_variable = netcdf.createVariable(
            data_variable_name,
            data_dtype,
...
# ----------------------------------------------------------------------------------------------------------------------
def add_variable_climdivs_divstime(
    file_path, variable_name, variable_attributes, divisions_to_arrays
):
    """
    ...
    with netCDF4.Dataset(file_path, "a") as dataset:
        ...
        if variable_name in dataset.variables.keys():
            ...
            message = "Variable name '{0}' is already being used within the NetCDF file '{1}'".format(
        ...
        variable = dataset.createVariable(
            variable_name, netcdf_data_type, ("division", "time"), fill_value=np.NaN
        )
        ...
        for division_index, division_id in enumerate(
            list(dataset.variables["division"][:])
        ):
            ...
            if division_id in divisions_to_arrays.keys():
                ...
                if data_array.size == times_size:
                    ...
                    variable[division_index, :] = np.reshape(
                        data_array, (1, times_size)
                    )

                else:

                    _logger.info(
                        "Unexpected size of data array for division ID {0} -- ".format(
                            division_id
                        )
                        + "expected {0} time steps but the array contains {1}".format(
                            times_size, data_array.size
                        )
                    )


# ----------------------------------------------------------------------------------------------------------------------
def add_variable_climdivs_divs(
    file_path, variable_name, variable_attributes, divisions_to_values
):
    ...
    with netCDF4.Dataset(file_path, "a") as dataset:
        ...
        # create the variable, set the attributes
        variable = dataset.createVariable(
            variable_name, netcdf_data_type, ("division",), fill_value=np.NaN
        )
        variable.setncatts(variable_attributes)
...
# ----------------------------------------------------------------------------------------------------------------------
def initialize_variable_climdivs(
    netcdf,  # pragma: no cover
    data_variable_name,
