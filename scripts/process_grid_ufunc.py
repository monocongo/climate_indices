...
def compute_write_spi(kwrgs):
    ...
    spi_var = xr.Variable(dims=da_spi.dims, data=da_spi, attrs=spi_attrs)
...
def compute_write_pnp(kwrgs):
    ...
    _logger.info(
        "Computing {scale}-{incr} {index}".format(
            scale=kwrgs["scale"], incr=scale_increment, index="PNP"
        )
    )
    ...
    da_pnp = xr.apply_ufunc(
        indices.percentage_of_normal, da_precip_groupby, kwargs=args_dict
    )
    ...
    long_name = "Percentage of Normal Precipitation" + "{scale}-{increment}".format(
        scale=kwrgs["scale"], increment=scale_increment
    )
    pnp_attrs = {"long_name": long_name, "valid_min": -1000.0, "valid_max": 1000.0}
    var_name_pnp = "pnp_" + "_" + str(kwrgs["scale"]).zfill(2)
    pnp_var = xr.Variable(dims=da_pnp.dims, data=da_pnp, attrs=pnp_attrs)
...
def compute_write_spei(kwrgs):
    ...
    da_spei = xr.apply_ufunc(
        indices.spei, da_precip_groupby, da_pet_groupby, kwargs=args_dict
    )
    ...
    spei_attrs = {"long_name": long_name, "valid_min": -3.09, "valid_max": 3.09}
    var_name_spei = (
        "spei_" + kwrgs["distribution"].value + "_" + str(kwrgs["scale"]).zfill(2)
    )
    spei_var = xr.Variable(dims=da_spei.dims, data=da_spei, attrs=spei_attrs)
...
def compute_write_palmers(kwrgs):
    ...
    da_scpdsi, da_pdsi, da_phdi, da_pmdi, da_zindex = xr.apply_ufunc(
        indices.scpdsi,
        da_precip_groupby,
        da_pet_groupby,
        da_awc_groupby,
    ...
    scpdsi_var = xr.Variable(dims=da_scpdsi.dims, data=da_scpdsi, attrs=scpdsi_attrs)
    ...
    var_name_pdsi = "pdsi"
    pdsi_var = xr.Variable(dims=da_pdsi.dims, data=da_pdsi, attrs=pdsi_attrs)
    ...
    var_name_phdi = "phdi"
    phdi_var = xr.Variable(dims=da_phdi.dims, data=da_phdi, attrs=phdi_attrs)
    ...
    var_name_pmdi = "pmdi"
    pmdi_var = xr.Variable(dims=da_pmdi.dims, data=da_pmdi, attrs=pmdi_attrs)
    ...
    zindex_var = xr.Variable(dims=da_zindex.dims, data=da_zindex, attrs=zindex_attrs)
...
def compute_write_pet(kwrgs):
    ...
    da_pet = xr.apply_ufunc(
        indices.pet, da_temp_groupby, da_lat_groupby, kwargs=args_dict
    )
    ...
    pet_var = xr.Variable(dims=da_pet.dims, data=da_pet, attrs=pet_attrs)
