...
#                             print('{0}  Expected:  {1}   Actual: {2}'.format(i, expected[i], actual[i]))
                ...
                pdinew_pdat, pdinew_spdat, pdinew_pedat, pdinew_pldat, pdinew_prdat, pdinew_rdat, pdinew_tldat, pdinew_etdat, pdinew_rodat, pdinew_prodat, pdinew_tdat, pdinew_sssdat, pdinew_ssudat = \
                ...
                palmer_ET, palmer_PR, palmer_R, palmer_RO, palmer_PRO, palmer_L, palmer_PL = palmer._water_balance(awc + 1.0, palmer_PE, precip_timeseries)
                   
                # compare the values against the values produced monthly by pdinew.py
                etdat_diffs = palmer_ET - pdinew_etdat.flatten()
                tldat_diffs = palmer_L - pdinew_tldat.flatten()
                pldat_diffs = palmer_PL - pdinew_pldat.flatten()
                rdat_diffs = palmer_R - pdinew_rdat.flatten()
                prdat_diffs = palmer_PR - pdinew_prdat.flatten()
                rodat_diffs = palmer_RO - pdinew_rodat.flatten()
                prodat_diffs = palmer_PRO - pdinew_prodat.flatten()
