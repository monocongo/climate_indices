...
from indices_python import indices, ingest, netcdf_utils
import logging
...
#-----------------------------------------------------------------------------------------------------------------------
def _plot_and_save_histogram(diffs,
                             number_of_bins,
                             range_lower, 
                             range_upper,
                             index,
    ...
    # save to file
    filepath = output_filepath + '_{0}_{1}.png'.format(index, division_id)
    logger.info('Saving histogram plot for index {0} to file {1}'.format(index, filepath))
...
if __name__ == '__main__':
    ...
    try:
        ...
        indices_python.ingest.ingest_netcdf_latest(args.out_file,
                                                   temp_var_name,
                                                   precip_var_name,
                                                   awc_var_name)
